...
class GiftWithPurchase extends HTMLElement {
  ...
  init() {
    const cart = document.getElementById("main-cart-items");
    cart.addEventListener("change", this.getGWPInfo);
  ...
  getGWPInfo = async () => {
    ...
    fetch("http://127.0.0.1:2053/gift-with-purchase", {
      ...
      body: JSON.stringify(collectionsAmount),
    })
      .then((resp) => resp.json())
      .then(this.updateGWPForm);
  };

  generateForm(productId, productDescription) {
    ...
    desc.style = "display: inline-block; margin-right: 16px";
    desc.textContent = productDescription.replace(" - Default Title", "");
  ...
  updateGWPForm = (gifts) => {
    const forms = [];
    const wrapper = document.querySelector(".gwp-wrapper");
    gifts.forEach((gift) => {
      const id = gift.giftId.replace("gid://shopify/ProductVariant/", "");
      forms.push(this.generateForm(id, gift.title));
    });
    wrapper.replaceChildren(...forms);
  };

  async getCollectionAmount() {
    await timeout(3000);
    const elements = document.querySelectorAll("[data-collection]");
    const collectionAndAmount = [];
    elements.forEach((element) => {
      const price = element.querySelector(".price--end");
      collectionAndAmount.push({
        collection: element.dataset.collection,
        amount: Number(price.textContent.replace("$", "")),
      });
    });
    return collectionAndAmount;
  }
}
